...
2. **Harvest** (repo root = `FilePipe/`):
   ...
   If your new name is built only from variables (no string literal the regex can see), either add a literal in Kotlin (e.g. a comment line is NOT enough) or add a small explicit list in `harvest_ligatures.py` (see existing patterns: ternary `name = if`, enum `FOO -> "…"`, etc.).
